...
# Cache the data loading to avoid reloading every rerun
@st.cache_data
def load_data():
    ...
    disease_map = {disease: i for i, disease in enumerate([
    ...
    train_df.replace({'prognosis': disease_map}, inplace=True)
...
@st.cache_data
def train_model(train_df, l1):
    X = train_df[l1]
    y = np.ravel(train_df[["prognosis"]])
    gnb = MultinomialNB()
    gnb.fit(X, y)
    return gnb
